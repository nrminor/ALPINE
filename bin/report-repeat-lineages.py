...
def main():
    ...
    cluster_df = pd.read_csv(
        cluster_table_path, sep="\t", header=None, na_values=["", "NA"]
    )
    ...
    repeat_clusts = pd.DataFrame(
        {"Accession": cluster_df[8], "Repeat Cluster Number": cluster_df[1]}
    )

    # Collate metadata about these new clusters
    metadata = pd.read_csv(metadata_path, delimiter="\t", na_values=["", "NA"])
    merged_metadata = pd.merge(metadata, repeat_clusts, on="Accession", how="inner")
    merged_metadata = merged_metadata.loc[
