...
pango_report <- read_csv(args[1], show_col_types = FALSE, trim_ws = TRUE)
...
# place code inside a pre-compiled function
main <- cmpfun(function(pango_report, available_cores){
  ...
  clusterEvalQ(cluster, c(library(tidyverse), library(cdcfluview), library(outbreakinfo)))
  ...
  # export candidates
  new_table <- pango_report %>%
    filter(anachronistic == TRUE) %>%
    select(-anachronistic)
  write_tsv(new_table, "anachronistic_sequence_candidates.tsv", na = "")
  
})

# run the compiled code
main(pango_report, available_cores)
